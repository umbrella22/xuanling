...
Bundle 会在所选 profile 内安装 `@xuanling-rs/xuanling-mcp@0.4.0`，并从该 profile 解析 schema adapter
和 JS launcher。Launcher 会在启动前校验当前平台的原生 package；不需要全局 package，也
不会在安装时下载 binary。

Server 只在第一次有效 Memory 调用时延迟打开 `~/.xuanling/memory.db`；启动、catalog 发现和格式
错误的 Memory 输入不会创建或迁移数据库。

Bundle 自带的 lazy wrapper 会让官方 bridge 遍历并缓存完整九工具 MCP profile，但起初只向模型
显示 `mcp_catalog__xuanling`。应先检索这个 Host 控制工具，并在每次调用中激活一个下一步所需的
精确 Memory raw name；随后它会作为常规 `mcp__xuanling__*` 工具出现。Lazy projection 会降低
初始 schema 成本，但不会从 server 合同中移除任何生命周期操作。
...
可以同时安装 `@xuanling-rs/xuanling-dsh-skills@0.4.0`，获得 proposal-first Memory 工作流和严格整文件
