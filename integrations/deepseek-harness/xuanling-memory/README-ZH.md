...
Bundle 会在所选 profile 内安装 `@xuanling-rs/xuanling-mcp@0.2.3`，并从该 profile 解析 schema adapter
和 JS launcher。Launcher 会在启动前校验当前平台的原生 package；不需要全局 package，也
不会在安装时下载 binary。

可以同时安装 `@xuanling-rs/xuanling-dsh-skills@0.2.3`，获得 proposal-first Memory 工作流和严格整文件
