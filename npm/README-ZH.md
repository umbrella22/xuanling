...
本目录维护 XuanLing MCP 0.2.6 的 npm 分发与发布自动化。完整发行版包含八个不可变 npm
...
| 安装别名 | 发布版本 | 平台 |
| --- | --- | --- |
| `@xuanling-rs/xuanling-mcp` | `0.2.6` | 稳定 Node.js 启动器 |
| `@xuanling-rs/xuanling-mcp-darwin-arm64` | `0.2.6-darwin-arm64` | macOS Apple Silicon |
| `@xuanling-rs/xuanling-mcp-linux-x64-gnu` | `0.2.6-linux-x64-gnu` | Linux x64，glibc 2.35 或更高版本 |
...
| Package | 作用 |
| --- | --- |
| `@xuanling-rs/xuanling-dsh-memory@0.2.6` | 带 DSH schema projection 的完整 Memory v2 profile |
| `@xuanling-rs/xuanling-dsh-skills@0.2.6` | 文件与 Memory 工作流 Skill 以及严格 overwrite policy |
| `@xuanling-rs/xuanling-dsh-tools@0.2.6` | 增量挂载完整 XuanLing catalog |
| `@xuanling-rs/xuanling-dsh-tools-replace@0.2.6` | 完整 catalog，并停用模型可见的原生文件系统工具行 |
...
- XuanLing 0.2.6 不声明 Developer ID 或 Authenticode 发布者签名。后续版本可以增加这些签名，
