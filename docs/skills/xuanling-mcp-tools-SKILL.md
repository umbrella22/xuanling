...
- **Shared lexical memory** — SQLite-backed phrase/token AND/token OR recall across unicode61 and
  trigram channels is RRF-merged and deterministically reranked. It works without an embedding
  model; Chinese/mixed text needs no download. Cross-project, cross-client.
...
- `memory_search`: literal QueryPlan channels (phrase, token AND/OR, unicode61, trigram) merge via
